...
var vkSafe = (function() {
  var accept_invite, button, button_click_text, decrypt, encrypt, generate_key, generate_safe_form, get_id, get_invite_text, handler, history, history_on_update, invite_msg, parse_url, processed, publ, rsa, safe_send, send_invite,
  ...
  handler = function(message, self) {
    var content, line;
    if (content[1] === 'INVITE' && !self) {
      if (!window.waiting_for_secure) {
    ...
    if (content[1] === 'MESSAGE') {
      try {
        ...
        return message.style.backgroundColor = '#ffe3e3';
      }
    } else if (content[1] === 'INVITE') {
      return message.innerText = '[vkSafe] Заявка на шифрование отправлена.';
    }
  };
